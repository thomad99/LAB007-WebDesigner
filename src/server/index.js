...
Style: Professional website mockup, clean design, modern UI, business-appropriate, realistic content placement`;
    ...
    console.error('Error generating mockup (detail):', error);
    await pool.query(
      'UPDATE jobs SET status = $1 WHERE id = $2',
      [`error: ${error.message}`, jobId]
    );
        ...
        <script>
          document.addEventListener('DOMContentLoaded', function() {
            ...
            form.addEventListener('submit', async function(e) {
              ...
              statusBox.innerHTML = '<p>🚀 Starting the redesign process...</p>';
            ...
            async function pollStatus(jobId) {
              try {
                ...
                statusBox.innerHTML = '<p>' + statusIcon + ' Status: ' + statusText + '</p>';
                ...
                if (data.status && !data.status.startsWith('error') && data.status !== 'completed') {
                  ...
                  if (data.mockupUrl) {
                    ...
                    statusBox.innerHTML += '<div class="demo-links"><h3>🚀 Your Redesigned Website:</h3>' + 
                      data.demoUrls.map(url => '<a href="' + url + '" target="_blank">View Redesigned Site</a>').join('') +
                      '</div>';
                  }
